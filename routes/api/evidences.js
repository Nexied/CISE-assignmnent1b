...
// @access public
router.get('./test', (req, res) => res.send('evidence route working'));
...
router.post('/:', (req, res) =>{
    ...
    .then(evidence => res.json({msg: 'Evidence added successfully'}))
    .catch(err => res.status(404).json({ error: 'unable to add this evidence'}));
});
...
router.put('/:id', (req, res) =>{
    ...
    .catch(err => res.status(404).json({ error: 'No evidence found'}));
});
...
router.delete('/:id', (req, res) =>{
    ...
    .then(evidence => res.json({msg: 'Evidence entry deleted successfully'}))
    .catch(err => res.status(404).json({ noevidencefound: 'No evidence found'}));
});
